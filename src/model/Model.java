...
/**
 * model.Model superclass, never instantiated. All child model classes inherits
 * its properties, classes and methods
 */
public abstract class Model {

	private static Configurations cf = new Configurations();

	private static String sqlUrl = "jdbc:mysql://" + cf.getHost() + ":"
			+ cf.getPort() + "/";
	 ...
	 */
	protected void readfromSqlFile(String filepath) throws IOException,
			SQLException {
		getConnection(true);
	...
	public void getConnection(Boolean init) throws SQLException {
		if (init) {
			...
			setConn(DriverManager.getConnection(sqlUrl + dbName, sqlUser,
					sqlPasswd));
		}
	}
